...
class LossLoggerCallback(TrainerCallback):
    def __init__(self, log_interval, output_dir):
...
def main():
    ...
    def transform_references(references):
        ...
        for ref in references:
            ref_dict = ref.copy()
            answers = ast.literal_eval(ref_dict["answers"])
            ref_dict["answers"] = {
                "text": answers["text"],
                "answer_start": answers["answer_start"]
            }
            transformed.append(ref_dict)
        return transformed
    ...
    def transform_predictions(predictions):
        ...
        for pred in predictions:
            pred_dict = pred.copy()
            pred_dict["prediction_text"] = pred_dict["prediction_text"][0]  # List에서 문자열로 변환
            transformed.append(pred_dict)
        return transformed

    def compute_metrics(p: EvalPrediction):
        references = [p.label_ids[i] for i in range(len(p.label_ids))]
        predictions = [p.predictions[i] for i in range(len(p.predictions))]
    ...
    # Add the LossLoggerCallback to the trainer
    loss_logger = LossLoggerCallback(log_interval=100, output_dir=training_args.output_dir)
